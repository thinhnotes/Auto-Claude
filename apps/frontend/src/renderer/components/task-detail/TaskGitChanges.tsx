...
export function TaskGitChanges({ task }: TaskGitChangesProps) {
  ...
  const loadGitChanges = useCallback(async () => {
    if (!task.projectId || !task.specId) return;

    setIsLoadingChanges(true);
    setChangesError(null);

    try {
      const result = await window.electronAPI.getTaskGitChanges(task.projectId, task.specId);
  ...
  const loadFileDiff = useCallback(async (filePath: string) => {
    ...
    try {
      const result = await window.electronAPI.getTaskFileDiff(task.projectId, task.specId, filePath);
      if (!result.success || !result.data) {

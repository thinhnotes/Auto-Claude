...
async def azure_devops_request(
    url: str, pat: str, method: str = "GET", body: dict | None = None
) -> dict[str, Any]:
...
async def get_iterations(project_id: str):
    ...
    if (
        ...
        or not config.organizationUrl
        or not config.project
        or not config.personalAccessToken
    ):
...
async def get_work_items_for_iteration(
    project_id: str, iterationPath: str, areaPath: str | None = None
):
    ...
    try:
        ...
        # Execute WIQL query
        wiql_url = (
            f"{config.organizationUrl}/{config.project}/_apis/wit/wiql?api-version=7.1"
        )
...
async def get_areas(project_id: str):
    ...
    try:
        ...
        def extract_areas(node, parent_path=""):
            ...
            items.append({"id": node.get("id"), "name": node.get("name"), "path": path})
